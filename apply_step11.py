"""
apply_step11.py -- parks idle capital in the SPY benchmark instead of flat
cash between trades. A real run found capital was only ever in an individual
stock pick 7.1% of the time -- this fixes that directly. Overwrites the same
3 files the last several apply_stepN.py scripts touched -- run from the repo
root:

    python3 apply_step11.py
    python3 -m pytest -q      # expect 120 passed
...
# change this back to {"BUY", "STRONG BUY"} to restore "trade every BUY"
# for comparison.
...
def main() -> None:
    ...
    print(
        ...
        f"Entering only on: {entry_label}. Idle capital: {idle_cash_label}...\n"
    )

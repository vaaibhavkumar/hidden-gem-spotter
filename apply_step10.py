"""
apply_step10.py -- adds capital-utilization and per-trade-return diagnostics
to the portfolio simulation, so a weak CAGR's cause (bad trades vs. barely
being invested at all) can be told apart from real numbers instead of guessed.
Overwrites the same 3 files apply_step8.py/apply_step9.py touched -- run from
the repo root:

    python3 apply_step10.py
    python3 -m pytest -q      # expect 113 passed
...
class PortfolioResult:
    ...
    def average_utilization(self) -> float | None:
        ...
        ever actually in a position, the rest sat in cash earning
        nothing. This exists to separate two very different explanations
        for a weak CAGR: bad trades (a low avg_trade_return) vs. barely
        being invested at all (a low average_utilization) -- the fix for
        each is completely different, and a hit rate or a raw CAGR alone
        can't tell them apart. Returns None if there's no time span to
        weight over (fewer than 2 recorded occupancy points).
        """
...
# baseline -- badly short of target, and 97.8% of trades never even hit
# a SELL signal, they just timed out at the horizon. Trading only the
# highest-conviction STRONG BUY signal (fewer, more selective entries)
# is the first thing being tried to see if conviction filtering closes
# any of that gap -- change this back to {"BUY", "STRONG BUY"} to
# restore the original "trade every BUY" behavior for comparison.
...
def main() -> None:
    ...
    print(
        ...
        f"Entering only on: {entry_label}...\n"
    )
    result = simulate_portfolio(signaled, max_concurrent=MAX_CONCURRENT, entry_actions=ENTRY_ACTIONS)
    ...
    if utilization is not None:
        print(
            f"Avg capital deployed:  {utilization * 100:.1f}% of the {MAX_CONCURRENT}-slot cap "
            f"(the rest sat in cash earning nothing -- this is the other half of the CAGR story, "
            f"separate from whether the trades themselves were any good)"
        )
    else:
